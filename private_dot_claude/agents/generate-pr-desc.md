...
description: 現在のブランチと base ブランチの差分から GitHub PR の説明マークダウンを生成し，クリップボードにコピーする
model: sonnet
tools:
  - Bash(git *)
  - Bash(pbcopy)
...
現在のブランチと base ブランチの差分を分析して，GitHub PR の説明マークダウンを生成し，クリップボードにコピーする．

引数で言語を指定できる：
- `ja` または `japanese` → 日本語で生成
- `en` または `english` → 英語で生成（デフォルト）

引数が指定されていない場合は英語で生成する．
